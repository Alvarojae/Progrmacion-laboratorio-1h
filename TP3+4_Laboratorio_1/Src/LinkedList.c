...
int ll_remove(LinkedList* this,int index)
{
    ...
    if(this != NULL && (index>=0 && index < ll_len(this)))
	{
    	auxDelete = getNode(this,index);
    	if(auxDelete!=NULL)
    	{
    		auxBeforeNode = getNode(this,index-1);

    		if(auxBeforeNode!=NULL)
    		{
    			auxBeforeNode->pNextNode = auxDelete->pNextNode;
    		}
			else
			{
				this->pFirstNode = auxDelete->pNextNode;
			}
